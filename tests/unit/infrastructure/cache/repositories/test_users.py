...
async def test_upsert_many_then_get_by_nick(
...
async def test_nick_display_falls_back_to_normalized_nick_when_unmapped(
    ...
    repo = UsersCacheRepository(connection)
    await repo.upsert_many([_profile()], nick_displays={}, synced_at=synced_at)
...
async def test_get_by_discord_id(connection: aiosqlite.Connection, synced_at: str) -> None:
    repo = UsersCacheRepository(connection)
    await repo.upsert_many([_profile(discord_id=123456)], nick_displays={}, synced_at=synced_at)
...
async def test_get_by_discord_id_returns_none_when_unbound(
    ...
    repo = UsersCacheRepository(connection)
    await repo.upsert_many([_profile(discord_id=None)], nick_displays={}, synced_at=synced_at)
...
async def test_upsert_many_updates_existing_row(
    ...
    repo = UsersCacheRepository(connection)
    await repo.upsert_many([_profile(coins=1)], nick_displays={}, synced_at=synced_at)
