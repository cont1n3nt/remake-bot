...
### Разовый шаг после обновления до миграции 0011 (плакаты)

Раскладка плакатов переехала из файлов внутри пакета в базу (заявка 13.09.2026 п.6), чтобы
её можно было править командой `/poster_item`, а не правкой кода. Миграция создаёт пустые
таблицы — наполнить их текущей раскладкой и перенести иконки нужно один раз, вручную:

```bash
python scripts/import_poster_layouts.py --db-path ./data/cache.sqlite3 --icons-dir ./data/poster_icons
```

Скрипт переносит 224 позиции из `layout_*.json` и копирует их иконки в `POSTER_ICONS_DIR`.
Повторный запуск откажется работать — чтобы переимпортировать заново, добавьте `--replace`.
**До запуска скрипта `/poster` нарисует пустые плакаты**: таблицы пустые, рисовать нечего.
